      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-12 border-b border-[#1F2937] pb-6">
            <div>
                ...
                <p className="text-[#94A3B8] font-mono text-sm mt-1">ACTIVE WORK ORDERS & FIELD MISSIONS</p>
            </div>
            <div className="text-right hidden md:block">
                <div className="text-[10px] uppercase text-[#94A3B8] font-mono">Order Count</div>
            ...
            >
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pl-4">
                ...
                <div className="flex-1">
                  <div className="font-mono text-xs text-[#00B8D4] mb-1">
                  ...
                  <div className="text-xl font-bold text-white group-hover:text-[#00E676] transition uppercase tracking-wide">
                    {ticket.pothole.roadInfo?.roadName || "UNKNOWN_SECTOR"}
                  </div>
                  <div className="text-xs text-[#94A3B8] font-mono mt-1 flex gap-4">
                     {new Date(ticket.createdAt).toLocaleDateString()} <span className="opacity-50">|</span> 
                     LAT: {ticket.pothole.latitude.toFixed(4)}...
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 md:gap-8">
                     ...
                     <div className="text-right">
                        <div className="text-[10px] font-bold text-[#94A3B8] uppercase">Assigned Agent</div>
                        <div className="font-mono text-white text-sm">
                            {ticket.assignedWorker ? ticket.assignedWorker.name : <span className="text-[#FF1744] italic">UNASSIGNED</span>}
                        </div>
                     </div>

                    <div className="text-right">
                        <div className="text-[10px] font-bold text-[#94A3B8] uppercase">Threat Level</div>
                         <div className={`font-black uppercase italic ${
                             ticket.pothole.priorityLevel === 'CRITICAL' ? 'text-[#FF1744]' : 
                             ticket.pothole.priorityLevel === 'HIGH' ? 'text-[#FF9100]' : 'text-[#00E676]'
                         }`}>
                             {ticket.pothole.priorityLevel || 'N/A'}
                         </div>
                    </div>

                    <div className={`px-3 py-1.5 rounded textxs font-bold uppercase border tracking-wider ${
                        ticket.status === 'RESOLVED' ? 'border-[#00E676] text-[#00E676] bg-[#00E676]/10' :
                        ticket.status === 'IN_PROGRESS' ? 'border-[#00B8D4] text-[#00B8D4] bg-[#00B8D4]/10' :
